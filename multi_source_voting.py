...
"""
#测试生成的模型
check_point = torch.load("target_classifier_itself.tar")
...
#结合target域上训练数据的辅助作用来判断多域的效果 (改过的)
target_label_dict = {}
target_train_dataset = TrainData("Multivariate_ts", "StandWalkJump/StandWalkJump_TRAIN.ts",target_label_dict)
target_test_dataset = TestData("Multivariate_ts", "StandWalkJump/StandWalkJump_TEST.ts",target_label_dict)
...
target_feature_extraction_module3 = target_feature_extraction_module3.cuda()
target_classification_module3 = target_classification_module3.cuda()
checkpoint1 = torch.load("epoch_10.tar")
checkpoint2 = torch.load("epoch_82.tar")
checkpoint3 = torch.load("epoch_280.tar")
target_feature_extraction_module1.load_state_dict(checkpoint1['feature_extraction_state_dict'])
target_classification_module1.load_state_dict(checkpoint1['classification_state_dict'])
target_feature_extraction_module1.eval()
target_classification_module1.eval()
...
with torch.no_grad():
    ...
    for i in range(target_num_class):
        ...
        for n in range(len(results_of_train1)):
            if results_of_train1[n] == i:
                num_for_this_label += 1
                if results_of_train1[n] == label_list_train[n]:
                    correct_for_this_label += 1
        if num_for_this_label != 0:
            weight_for_1.append(correct_for_this_label / num_for_this_label)
        else:
            weight_for_1.append(0)
    ...
    for i in range(target_num_class):
        num_for_this_label = 0
        correct_for_this_label = 0
        for n in range(len(results_of_train2)):
            if results_of_train2[n] == i:
                num_for_this_label += 1
                if results_of_train2[n] == label_list_train[n]:
                    correct_for_this_label += 1
        if num_for_this_label != 0:
            weight_for_2.append(correct_for_this_label / num_for_this_label)
        else:
            weight_for_2.append(0)
    ...
    for i in range(target_num_class):
        num_for_this_label = 0
        correct_for_this_label = 0
        for n in range(len(results_of_train3)):
            if results_of_train3[n] == i:
                num_for_this_label += 1
                if results_of_train3[n] == label_list_train[n]:
                    correct_for_this_label += 1
        if num_for_this_label != 0:
            weight_for_3.append(correct_for_this_label / num_for_this_label)
        else:
            weight_for_3.append(0)
...
for i in range(len(results_of_probs1)):
    ...
    results_of_probs1[i] = results_of_probs1[i] * (1+120*np.exp(-the_entropy)) * np.power(9,weight_1)
for i in range(len(results_of_probs2)):
    results_of_probs2[i] = np.exp(results_of_probs2[i]) / np.sum(np.exp(results_of_probs2[i]))
    the_entropy = entropy(results_of_probs2[i])
    #the_label = np.argmax(results_of_probs2[i])
    results_of_probs2[i] = results_of_probs2[i] * (1+120*np.exp(-the_entropy)) * np.power(9,weight_2)
for i in range(len(results_of_probs3)):
    results_of_probs3[i] = np.exp(results_of_probs3[i]) / np.sum(np.exp(results_of_probs3[i]))
    the_entropy = entropy(results_of_probs3[i])
    #the_label = np.argmax(results_of_probs3[i])
    results_of_probs3[i] = results_of_probs3[i] * (1+120*np.exp(-the_entropy)) * np.power(9,weight_3)
...
a = np.load("282_test_predict.npy")
b = np.load("446_test_predict.npy")
f = np.load("232_test_predict.npy")

d = np.load("232_test_true.npy")
...
im = Image.fromarray(np.array(d1).astype(np.uint8).reshape(22,-1))
im = im.convert('P')
im.putpalette(palette)
im.save('0_out.png')
im2 = Image.fromarray(np.array(d2).astype(np.uint8).reshape(22,-1))
im2 = im2.convert('P')
im2.putpalette(palette)
im2.save('1_out.png')
im3 = Image.fromarray(np.array(d3).astype(np.uint8).reshape(22,-1))
im3 = im3.convert('P')
